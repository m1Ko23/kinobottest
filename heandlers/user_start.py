...
async def cmd_start(message: types.Message):
    if message.chat.type == types.ChatType.PRIVATE:
        try:
            ...
            await bot.send_message(chat_id=message.from_user.id, text=f'[{message.from_user.full_name}](tg://user?id={message.from_user.id})', parse_mode=types.ParseMode.MARKDOWN)
        except:
            pass
